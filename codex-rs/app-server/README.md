...
- `mcpServerStatus/list` — enumerate configured MCP servers with their tools, auth status, and resolved `environmentId`, plus resources/resource templates for `full` detail; supports cursor+limit pagination. If `detail` is omitted, the server defaults to `full`.
